...
import { flightCreationDTO, flightDTO, flightFormCreationDTO } from 'src/app/Interfaces/flight';
...
export class FormFlightComponent implements OnInit{
  ...
  @Output()
  OnSubmit: EventEmitter<flightCreationDTO> = new EventEmitter<flightCreationDTO>();
  ...
  ngOnInit(): void {
    ...
    if(this.modelo !== undefined){
      console.log("iF ngOnInit");
      // Extraer la fecha y la hora de departureTime y arrivalTime
      const departureDate = new Date(this.modelo.departureTime);
      const departureTime = departureDate.toTimeString().split(' ')[0]; // Solo la hora
      const arrivalDate = new Date(this.modelo.arrivalTime);
      const arrivalTime = arrivalDate.toTimeString().split(' ')[0]; // Solo la hora
  
      // Crear un objeto con las fechas y horas separadas para hacer el patchValue
      const flight: Partial<flightFormCreationDTO> = {
        airline: this.modelo.airline,
        flightNumber: this.modelo.flightNumber,
        origin: this.modelo.origin,
        destination: this.modelo.destination,
        departureDate: departureDate, // Solo la fecha
        departureTime: departureTime, // Solo la hora
        arrivalDate: arrivalDate,     // Solo la fecha
        arrivalTime: arrivalTime,     // Solo la hora
        price: this.modelo.price,
        totalSeats: this.modelo.totalSeats
      };

      
      this.addressForm.patchValue(flight as Partial<typeof this.addressForm.value>);
